...
//    private LiveData<List<Schedule>> mDateSchedules;
    ...
    public void deleteAlarmByRequestId(int requestId) {
        try {
            new deleteAlarmByRequestIdAsyncTask(mRoomDatabase.alarmDao()).execute(requestId).get();
        } catch (InterruptedException e) {
            e.printStackTrace();
        } catch (ExecutionException e) {
            e.printStackTrace();
    ...
    public class deleteAlarmByRequestIdAsyncTask extends AsyncTask<Integer, Void, Void> {
        ...
        protected Void doInBackground(Integer... integers) {
            alarmDao.deleteAlarmByRequestId(integers[0]);
            return null;
        }
    }
